...
function registerIpc() {
  ...
  ipcMain.handle('store:addCombatant', async (_event, sceneId, tokenId) => store.addCombatant(sceneId, tokenId))
  ipcMain.handle('store:removeCombatant', async (_event, combatantId) => store.removeCombatant(combatantId))
  ipcMain.handle('store:patchCombat', async (_event, worldId, patch) => store.patchCombat(worldId, patch))
  ipcMain.handle('store:patchCombatant', async (_event, combatantId, patch) => store.patchCombatant(combatantId, patch))
  ...
  ipcMain.handle('store:createItem', async (_event, worldId, payload) => store.createItem(worldId, payload))
  ipcMain.handle('store:patchItem', async (_event, itemId, patch) => store.patchItem(itemId, patch))
  ipcMain.handle('store:deleteItem', async (_event, itemId) => store.deleteItem(itemId))
